...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all(ids.map(id => fetch('http://localhost:8000/analyze/' + id, {method:'POST'}).then(r => r.json())))
      .then(setReports);
  }, []);
      ...
      <div style={{marginTop: '50px', maxWidth: '800px'}}>
        {reports.map((item, i) => (
          <div key={i} style={{background: '#0f172a', padding: '30px', borderRadius: '25px', marginBottom: '25px', border: '1px solid #1e293b'}}>
            <h2 style={{fontSize: '2rem', margin: '0 0 10px 0'}}>{item.report ? item.report.status : "Loading..."}</h2>
